' ResultsScreen.brs - End game results display (dynamically scaled)
...
sub initScreenDimensions()
    ...
    ' Scale factor based on 720p reference (matching other screens)
...
sub calculateLayout()
    ...
    ' Margins and padding
    m.marginH = int(m.screenWidth * 0.04)        ' ~4% horizontal margin
    m.marginV = int(m.screenHeight * 0.04)       ' ~4% vertical margin
    m.paddingSmall = int(10 * m.scaleFactor)
    m.paddingMed = int(15 * m.scaleFactor)
    m.paddingLarge = int(20 * m.scaleFactor)
    
    ' Header position
    m.headerY = int(m.screenHeight * 0.04)       ' ~4% from top
    
    ' Song info position (left column)
    m.songInfoX = m.marginH
    m.songInfoY = int(m.screenHeight * 0.12)     ' ~12% from top
    m.artistOffsetY = int(40 * m.scaleFactor)
    
    ' Grade display (center-right area)
    m.gradeSize = int(120 * m.scaleFactor)
    m.gradeX = int(m.screenWidth * 0.75) - (m.gradeSize / 2)  ' 75% from left, centered
    m.gradeY = int(m.screenHeight * 0.10)
    
    ' Panel layout - centered with equal margins
    ' Two panels each 42% wide with 4% gap between, centered on screen
    m.panelWidth = int(m.screenWidth * 0.42)
    m.panelGap = int(m.screenWidth * 0.04)       ' 4% gap between panels
    totalPanelsWidth = (m.panelWidth * 2) + m.panelGap
    m.panelMargin = int((m.screenWidth - totalPanelsWidth) / 2)  ' Equal left/right margins
    
    ' Score panel (left panel)
    m.scorePanelX = m.panelMargin
    m.scorePanelY = int(m.screenHeight * 0.28)   ' ~28% from top
    m.scorePanelWidth = m.panelWidth
    m.scorePanelHeight = int(180 * m.scaleFactor)
    
    ' Accuracy and combo sub-panels
    m.subPanelWidth = int((m.scorePanelWidth - m.paddingMed) / 2)
    m.subPanelHeight = int(70 * m.scaleFactor)
    m.subPanelY = int(m.screenHeight * 0.45)     ' Below main score
    
    ' Hit breakdown panel (right panel - gap centered at screen center)
    ...
    ' Calculate total buttons width for centering
...
sub setupReferences()
    ...
    ' Store button references for selection
...
sub updateButtonSelection()
    ' Colors for buttons
    selectedColor = "0x6c5ce7FF"    ' Purple - selected
    unselectedColor = "0x2d3436FF"  ' Dark gray - unselected
...
sub updateGrade(result as object)
    ...
    grade = "D"
    gradeColor = "0xd63031FF"  ' Red
    
    if accuracy >= 95 then
        grade = "S"
        gradeColor = "0xffeaa7FF"  ' Gold
    else if accuracy >= 90 then
        grade = "A"
        gradeColor = "0x00cec9FF"  ' Cyan
    else if accuracy >= 80 then
        grade = "B"
        gradeColor = "0x74b9ffFF"  ' Blue
    else if accuracy >= 70 then
        grade = "C"
        gradeColor = "0xfdcb6eFF"  ' Yellow
    end if
